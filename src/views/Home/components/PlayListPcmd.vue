<template>
  ...
  <BetterScroll :config="{ scrollX: true, scrollY: false }" :dep="data">
    <div
      v-for="item in data"
      class="title flex flex-col items-center w-[33vw] h-[50vw]"
    >
      <div class="pl-[10px]">
        <div class="w-[32vw] h-[32vw] relative">
          <img
            ...
            alt=""
            class="rounded-[10px] w-[30vw] h-[30vw]"
          />
          <div
            class="absolute w-full flex flex-row translate-x-[0.5rem] translate-y-[-6.7rem] text-[#f1f0ee]"
          >
            ...
            <div>
              {{ bigNumTransform(item.resourceExtInfo.playCount) }}
            </div>
          </div>
...
import { bigNumTransform } from "@/utils/bigNumtransform";
